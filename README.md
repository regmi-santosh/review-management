...
**Marketing site**: [website/index.html](website/index.html) is a standalone landing page (working name "Tend") — static HTML, no build step, unrelated to the Python system above. Open it directly in a browser or host it anywhere that serves static files. The "Request early access" button is a `mailto:` placeholder (`hello@tend.app`) — replace with a real address before sharing it anywhere.
...
2. Add `business.json`:
   ...
   Optional milestone settings (see [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) "Milestone layer"): `"milestone_thresholds": [50, 100, 250, 500, ...]` (total review counts worth a celebratory post — on by default with a generic list even if you omit this key), `"rating_streak_milestones": [5, 10, 25, 50]` (consecutive 5-star review counts — opt-in, no-op until set), and `"founded_date": "YYYY-MM-DD"` (enables yearly anniversary posts — opt-in, no-op until set; see [docs/ONBOARDING.md](docs/ONBOARDING.md) for when to add it).
