...
class MockGoogleBusinessProfileClient(GoogleBusinessProfileClient):
    """Stands in until Google Business Profile API access is granted for
    'Brows & Threading City' (see README)."""
    ...
    def __init__(self) -> None:
        missing = [
            ...
            for name, value in [
                ...
                ("GOOGLE_BUSINESS_ACCOUNT_ID", config.GOOGLE_BUSINESS_ACCOUNT_ID),
                ("GOOGLE_BUSINESS_LOCATION_ID", config.GOOGLE_BUSINESS_LOCATION_ID),
            ]
            if not value
        ]
        if missing:
            raise RuntimeError(
                "GOOGLE_CLIENT_MODE=live but missing required .env settings: "
                + ", ".join(missing)
